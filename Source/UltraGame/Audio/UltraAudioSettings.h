...
#include "UltraAudioSettings.generated.h"
...
USTRUCT()
struct ULTRAGAME_API FUltraSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "UltraAudioSettings"))
class ULTRAGAME_API UUltraAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FUltraSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FUltraSubmixEffectChainMap> LDRAudioSubmixEffectChain;
