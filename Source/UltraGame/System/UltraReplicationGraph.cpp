...
namespace Ultra::RepGraph
{
	...
	static FAutoConsoleVariableRef CVarUltraRepGraphDestructMaxDist(TEXT("Ultra.RepGraph.DestructInfo.MaxDist"), DestructionInfoMaxDist, TEXT("Max distance (not squared) to rep destruct infos at"), ECVF_Default);

	int32 DisplayClientLevelStreaming = 0;
	static FAutoConsoleVariableRef CVarUltraRepGraphDisplayClientLevelStreaming(TEXT("Ultra.RepGraph.DisplayClientLevelStreaming"), DisplayClientLevelStreaming, TEXT(""), ECVF_Default);

	float CellSize = 10000.f;
	static FAutoConsoleVariableRef CVarUltraRepGraphCellSize(TEXT("Ultra.RepGraph.CellSize"), CellSize, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarUltraRepGraphSpatialBiasX(TEXT("Ultra.RepGraph.SpatialBiasX"), SpatialBiasX, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarUltraRepSpatialBiasY(TEXT("Ultra.RepGraph.SpatialBiasY"), SpatialBiasY, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarUltraRepDynamicActorFrequencyBuckets(TEXT("Ultra.RepGraph.DynamicActorFrequencyBuckets"), DynamicActorFrequencyBuckets, TEXT(""), ECVF_Default);

	int32 DisableSpatialRebuilds = 1;
	static FAutoConsoleVariableRef CVarUltraRepDisableSpatialRebuilds(TEXT("Ultra.RepGraph.DisableSpatialRebuilds"), DisableSpatialRebuilds, TEXT(""), ECVF_Default);

	int32 LogLazyInitClasses = 0;
	static FAutoConsoleVariableRef CVarUltraRepLogLazyInitClasses(TEXT("Ultra.RepGraph.LogLazyInitClasses"), LogLazyInitClasses, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarUltraRepTargetKBytesSecFastSharedPath(TEXT("Ultra.RepGraph.TargetKBytesSecFastSharedPath"), TargetKBytesSecFastSharedPath, TEXT(""), ECVF_Default);

	float FastSharedPathCullDistPct = 0.80f;
	static FAutoConsoleVariableRef CVarUltraRepFastSharedPathCullDistPct(TEXT("Ultra.RepGraph.FastSharedPathCullDistPct"), FastSharedPathCullDistPct, TEXT(""), ECVF_Default);

	int32 EnableFastSharedPath = 1;
	static FAutoConsoleVariableRef CVarUltraRepEnableFastSharedPath(TEXT("Ultra.RepGraph.EnableFastSharedPath"), EnableFastSharedPath, TEXT(""), ECVF_Default);
	...
	UReplicationDriver* ConditionalCreateReplicationDriver(UNetDriver* ForNetDriver, UWorld* World)
	{
		...
		if (World && ForNetDriver && ForNetDriver->NetDriverName == NAME_GameNetDriver)
		{
			...
			if (UltraRepGraphSettings && UltraRepGraphSettings->bDisableReplicationGraph)
			{
				UE_LOG(LogUltraRepGraph, Display, TEXT("Replication graph is disabled via UltraReplicationGraphSettings."));
				return nullptr;
			}

			UE_LOG(LogUltraRepGraph, Display, TEXT("Replication graph is enabled for %s in world %s."), *GetNameSafe(ForNetDriver), *GetPathNameSafe(World));

			TSubclassOf<UUltraReplicationGraph> GraphClass = UltraRepGraphSettings->DefaultReplicationGraphClass.TryLoadClass<UUltraReplicationGraph>();
			if (GraphClass.Get() == nullptr)
			{
				GraphClass = UUltraReplicationGraph::StaticClass();
			}

			UUltraReplicationGraph* UltraReplicationGraph = NewObject<UUltraReplicationGraph>(GetTransientPackage(), GraphClass.Get());
			return UltraReplicationGraph;
		}
...
UUltraReplicationGraph::UUltraReplicationGraph()
{
	if (!UReplicationDriver::CreateReplicationDriverDelegate().IsBound())
	{
		UReplicationDriver::CreateReplicationDriverDelegate().BindLambda(
			...
			{
				return Ultra::RepGraph::ConditionalCreateReplicationDriver(ForNetDriver, World);
			});
	}
}

void UUltraReplicationGraph::ResetGameWorldState()
{
...
EClassRepNodeMapping UUltraReplicationGraph::GetClassNodeMapping(UClass* Class) const
{
	if (!Class)
	{
...
void UUltraReplicationGraph::RegisterClassRepNodeMapping(UClass* Class)
{
	EClassRepNodeMapping Mapping = GetClassNodeMapping(Class);
	ClassRepNodePolicies.Set(Class, Mapping);
}

void UUltraReplicationGraph::InitClassReplicationInfo(FClassReplicationInfo& Info, UClass* Class, bool Spatialize) const
{
	AActor* CDO = Class->GetDefaultObject<AActor>();
	if (Spatialize)
	{
		Info.SetCullDistanceSquared(CDO->NetCullDistanceSquared);
		UE_LOG(LogUltraRepGraph, Log, TEXT("Setting cull distance for %s to %f (%f)"), *Class->GetName(), Info.GetCullDistanceSquared(), Info.GetCullDistance());
	}
	...
	UE_LOG(LogUltraRepGraph, Log, TEXT("Setting replication period for %s (%s) to %d frames (%.2f)"), *Class->GetName(), *NativeClass->GetName(), Info.ReplicationPeriodFrame, CDO->NetUpdateFrequency);
}

bool UUltraReplicationGraph::ConditionalInitClassReplicationInfo(UClass* ReplicatedClass, FClassReplicationInfo& ClassInfo)
{
...
void UUltraReplicationGraph::AddClassRepInfo(UClass* Class, EClassRepNodeMapping Mapping)
{
	if (IsSpatialized(Mapping))
	{
		if (Class->GetDefaultObject<AActor>()->bAlwaysRelevant)
		{
			UE_LOG(LogUltraRepGraph, Warning, TEXT("Replicated Class %s is AlwaysRelevant but is initialized into a spatialized node (%s)"), *Class->GetName(), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
		}
	}
...
void UUltraReplicationGraph::RegisterClassReplicationInfo(UClass* ReplicatedClass)
{
	...
	if (ConditionalInitClassReplicationInfo(ReplicatedClass, ClassInfo))
	{
		...
		UE_LOG(LogUltraRepGraph, Log, TEXT("Setting %s - %.2f"), *GetNameSafe(ReplicatedClass), ClassInfo.GetCullDistance());
	}
}

void UUltraReplicationGraph::InitGlobalActorClassSettings()
{
	...
	GlobalActorReplicationInfoMap.SetInitClassInfoFunc(
		[this](UClass* Class, FClassReplicationInfo& ClassInfo)
		{
			...
			if (Ultra::RepGraph::LogLazyInitClasses != 0)
			{
				if (bHandled)
				{
					EClassRepNodeMapping Mapping = ClassRepNodePolicies.GetChecked(Class);
					UE_LOG(LogUltraRepGraph, Warning, TEXT("%s was Lazy Initialized. (Parent: %s) %d."), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()), (int32)Mapping);
					...
					{
						UE_LOG(LogUltraRepGraph, Warning, TEXT("Differences Found!"));
						FString DebugStr = ParentRepInfo.BuildDebugStringDelta();
						UE_LOG(LogUltraRepGraph, Warning, TEXT("  Parent: %s"), *DebugStr);

						DebugStr = ClassInfo.BuildDebugStringDelta();
						UE_LOG(LogUltraRepGraph, Warning, TEXT("  Class : %s"), *DebugStr);
					}
				}
				else
				{
					UE_LOG(LogUltraRepGraph, Warning, TEXT("%s skipped Lazy Initialization because it does not differ from its parent. (Parent: %s)"), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()));
	...
	for (const FRepGraphActorClassSettings& ActorClassSettings : UltraRepGraphSettings->ClassSettings)
	{
...
#if WITH_GAMEPLAY_DEBUGGER
	AddClassRepInfo(AGameplayDebuggerCategoryReplicator::StaticClass(), EClassRepNodeMapping::NotRouted);				// Replicated via UUltraReplicationGraphNode_AlwaysRelevant_ForConnection
#endif
	...
	CharacterClassRepInfo.SetCullDistanceSquared(AUltraCharacter::StaticClass()->GetDefaultObject<AUltraCharacter>()->NetCullDistanceSquared);
...

	{
		...
		FRepMovement DefaultRepMovement = AUltraCharacter::StaticClass()->GetDefaultObject<AUltraCharacter>()->GetReplicatedMovement(); // Use the same quantization settings as our default replicatedmovement
	...
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.NumBuckets = Ultra::RepGraph::DynamicActorFrequencyBuckets;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.BucketThresholds.Reset();
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.EnableFastPath = (Ultra::RepGraph::EnableFastSharedPath > 0);
	...
	UE_LOG(LogUltraRepGraph, Log, TEXT(""));
	UE_LOG(LogUltraRepGraph, Log, TEXT("Class Routing Map: "));
	for (auto ClassMapIt = ClassRepNodePolicies.CreateIterator(); ClassMapIt; ++ClassMapIt)
	{
		...
		UE_LOG(LogUltraRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(ParentNativeClass), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
	}

	UE_LOG(LogUltraRepGraph, Log, TEXT(""));
	UE_LOG(LogUltraRepGraph, Log, TEXT("Class Settings Map: "));
	...
	for (auto ClassRepInfoIt = GlobalActorReplicationInfoMap.CreateClassMapIterator(); ClassRepInfoIt; ++ClassRepInfoIt)
	{
		...
		UE_LOG(LogUltraRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(GetParentNativeClass(Class)), *ClassInfo.BuildDebugStringDelta());
	}


	// Rep destruct infos based on CVar value
	DestructInfoMaxDistanceSquared = Ultra::RepGraph::DestructionInfoMaxDist * Ultra::RepGraph::DestructionInfoMaxDist;
	...
	for (const FRepGraphActorClassSettings& ActorClassSettings : UltraRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddToRPC_Multicast_OpenChannelForClassMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(LogUltraRepGraph, Log, TEXT("ActorClassSettings -- RPC_Multicast_OpenChannelForClass - %s"), *StaticActorClass->GetName());
...
void UUltraReplicationGraph::InitGlobalGraphNodes()
{
	// -----------------------------------------------
	//	Spatial Actors
	// -----------------------------------------------
	...
	GridNode->CellSize = Ultra::RepGraph::CellSize;
	GridNode->SpatialBias = FVector2D(Ultra::RepGraph::SpatialBiasX, Ultra::RepGraph::SpatialBiasY);

	if (Ultra::RepGraph::DisableSpatialRebuilds)
	{
	...
	// -----------------------------------------------
	UUltraReplicationGraphNode_PlayerStateFrequencyLimiter* PlayerStateNode = CreateNewNode<UUltraReplicationGraphNode_PlayerStateFrequencyLimiter>();
	AddGlobalGraphNode(PlayerStateNode);
}

void UUltraReplicationGraph::InitConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection)
{
	Super::InitConnectionGraphNodes(RepGraphConnection);

	UUltraReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = CreateNewNode<UUltraReplicationGraphNode_AlwaysRelevant_ForConnection>();

	// This node needs to know when client levels go in and out of visibility
	RepGraphConnection->OnClientVisibleLevelNameAdd.AddUObject(AlwaysRelevantConnectionNode, &UUltraReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd);
	RepGraphConnection->OnClientVisibleLevelNameRemove.AddUObject(AlwaysRelevantConnectionNode, &UUltraReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove);

	AddConnectionGraphNode(AlwaysRelevantConnectionNode, RepGraphConnection);
}

EClassRepNodeMapping UUltraReplicationGraph::GetMappingPolicy(UClass* Class)
{
...
#if WITH_GAMEPLAY_DEBUGGER
void UUltraReplicationGraph::OnGameplayDebuggerOwnerChange(AGameplayDebuggerCategoryReplicator* Debugger, APlayerController* OldOwner)
{
	...
	if (UUltraReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(Debugger->GetReplicationOwner()))
	{
...
void UUltraReplicationGraphNode_AlwaysRelevant_ForConnection::ResetGameWorldState()
{
	ReplicationActorList.Reset();
	AlwaysRelevantStreamingLevelsNeedingReplication.Empty();
}

void UUltraReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	UUltraReplicationGraph* UltraGraph = CastChecked<UUltraReplicationGraph>(GetOuter());
	...
	for (const FNetViewer& CurViewer : Params.Viewers)
	{
		...
		if (AUltraPlayerController* PC = Cast<AUltraPlayerController>(CurViewer.InViewer))
		{
			...
			if (bReplicatePS)
			{
				// Always return the player state to the owning player. Simulated proxy player states are handled by UUltraReplicationGraphNode_PlayerStateFrequencyLimiter
			...
			if (AUltraCharacter* Pawn = Cast<AUltraCharacter>(PC->GetPawn()))
			{
			...
			if (AUltraCharacter* ViewTargetPawn = Cast<AUltraCharacter>(CurViewer.ViewTarget))
			{
	...
	for (int32 Idx = AlwaysRelevantStreamingLevelsNeedingReplication.Num() - 1; Idx >= 0; --Idx)
	{
		...
		if (RepList.Num() > 0)
		{
			...
			if (bAllDormant)
			{
				UE_CLOG(Ultra::RepGraph::DisplayClientLevelStreaming > 0, LogUltraRepGraph, Display, TEXT("CLIENTSTREAMING All AlwaysRelevant Actors Dormant on StreamingLevel %s for %s. Removing list."), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				AlwaysRelevantStreamingLevelsNeedingReplication.RemoveAtSwap(Idx, 1, false);
			}
			else
			{
				UE_CLOG(Ultra::RepGraph::DisplayClientLevelStreaming > 0, LogUltraRepGraph, Display, TEXT("CLIENTSTREAMING Adding always Actors on StreamingLevel %s for %s because it has at least one non dormant actor"), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				Params.OutGatheredReplicationLists.AddReplicationActorList(RepList);
			}
		}
		else
		{
			UE_LOG(LogUltraRepGraph, Warning, TEXT("UUltraReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection - empty RepList %s"), *Params.ConnectionManager.GetName());
		}
...
void UUltraReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd(FName LevelName, UWorld* StreamingWorld)
{
	UE_CLOG(Ultra::RepGraph::DisplayClientLevelStreaming > 0, LogUltraRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityAdd - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Add(LevelName);
}

void UUltraReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove(FName LevelName)
{
	UE_CLOG(Ultra::RepGraph::DisplayClientLevelStreaming > 0, LogUltraRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityRemove - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Remove(LevelName);
}

void UUltraReplicationGraphNode_AlwaysRelevant_ForConnection::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	...
	{
		UUltraReplicationGraph* UltraGraph = CastChecked<UUltraReplicationGraph>(GetOuter());
		if (FActorRepListRefView* RepList = UltraGraph->AlwaysRelevantStreamingLevelActors.Find(LevelName))
		{
...
UUltraReplicationGraphNode_PlayerStateFrequencyLimiter::UUltraReplicationGraphNode_PlayerStateFrequencyLimiter()
{
	bRequiresPrepareForReplicationCall = true;
}

void UUltraReplicationGraphNode_PlayerStateFrequencyLimiter::PrepareForReplication()
{
...
void UUltraReplicationGraph::PrintRepNodePolicies()
{
	...
	GLog->Logf(TEXT("===================================="));
	GLog->Logf(TEXT("Ultra Replication Routing Policies"));
	GLog->Logf(TEXT("===================================="));
