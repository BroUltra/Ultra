...
#include "UltraInputModifiers.generated.h"

class FProperty;
class UEnhancedPlayerInput;
class UUltraAimSensitivityData;
class UObject;
...
class UUltraSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the Ultra Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Ultra Settings Driven Dead Zone"))
class UUltraInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EUltraTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in Ultra Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Ultra Gamepad Sensitivity"))
class UUltraInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	EUltraTargetingType TargetingType = EUltraTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UUltraAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the Ultra Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Ultra Aim Inversion Setting"))
class UUltraInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
