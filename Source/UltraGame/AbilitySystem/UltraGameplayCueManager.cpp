...
enum class EUltraEditorLoadMode
{
...
namespace UltraGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Ultra.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via UltraGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UUltraGameplayCueManager::DumpGameplayCues));

	static EUltraEditorLoadMode LoadMode = EUltraEditorLoadMode::LoadUpfront;
}
...
UUltraGameplayCueManager::UUltraGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UUltraGameplayCueManager* UUltraGameplayCueManager::Get()
{
	return Cast<UUltraGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UUltraGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UUltraGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogUltra, Warning, TEXT("UUltraGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UUltraGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (UltraGameplayCueManagerCvars::LoadMode)
	{
	case EUltraEditorLoadMode::LoadUpfront:
		return true;
	case EUltraEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EUltraEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UUltraGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UUltraGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UUltraGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UUltraGameplayCueManager* GCM = Cast<UUltraGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogUltra, Error, TEXT("DumpGameplayCues failed. No UUltraGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogUltra, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogUltra, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogUltra, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogUltra, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogUltra, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogUltra, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogUltra, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogUltra, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogUltra, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogUltra, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogUltra, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogUltra, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UUltraGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UUltraGameplayCueManager* StrongThis = Get())
					{
...
void UUltraGameplayCueManager::HandlePostGarbageCollect()
{
...
void UUltraGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogUltra, Warning, TEXT("UUltraGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UUltraGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (UltraGameplayCueManagerCvars::LoadMode)
	{
	case EUltraEditorLoadMode::LoadUpfront:
...
void UUltraGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UUltraGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UUltraGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UUltraGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UUltraGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UUltraGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
