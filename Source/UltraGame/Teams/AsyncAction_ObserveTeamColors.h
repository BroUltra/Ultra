...
class IUltraTeamAgentInterface;
class UUltraTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UUltraTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements IUltraTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UUltraTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UUltraTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<IUltraTeamAgentInterface> TeamInterfacePtr;
