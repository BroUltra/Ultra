...
void UAsyncAction_QueryReplays::Activate()
{
	...
	ResultList = NewObject<UUltraReplayList>();
...
void UAsyncAction_QueryReplays::OnEnumerateStreamsComplete(const FEnumerateStreamsResult& Result)
{
	...
	{
		UUltraReplayListEntry* NewReplayEntry = NewObject<UUltraReplayListEntry>(ResultList);
	...
	{
		FORCEINLINE bool operator()(const UUltraReplayListEntry& A, const UUltraReplayListEntry& B) const
		{
