...
#include "AnimNotify_UltraContextEffects.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_UltraContextEffects)



UAnimNotify_UltraContextEffects::UAnimNotify_UltraContextEffects()
{
}

void UAnimNotify_UltraContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_UltraContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_UltraContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_UltraContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> UltraContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UUltraContextEffectsInterface>())
			{
				// If so, add it to the Array
				UltraContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UUltraContextEffectsInterface>())
					{
						UltraContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* UltraContextEffectImplementingObject : UltraContextEffectImplementingObjects)
			{
				if (UltraContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IUltraContextEffectsInterface::Execute_AnimMotionEffect(UltraContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UUltraContextEffectLibrary type
						if (UUltraContextEffectsLibrary* EffectLibrary = Cast<UUltraContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_UltraContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_UltraContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FUltraContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FUltraContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FUltraContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
