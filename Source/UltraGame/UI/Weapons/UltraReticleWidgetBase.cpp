...
#include "UltraReticleWidgetBase.h"
...
#include "Weapons/UltraRangedWeaponInstance.h"
#include "Weapons/UltraWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(UltraReticleWidgetBase)

UUltraReticleWidgetBase::UUltraReticleWidgetBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UUltraReticleWidgetBase::InitializeFromWeapon(UUltraWeaponInstance* InWeapon)
{
	WeaponInstance = InWeapon;
	InventoryInstance = nullptr;
	if (WeaponInstance)
	{
		InventoryInstance = Cast<UUltraInventoryItemInstance>(WeaponInstance->GetInstigator());
	}
	OnWeaponInitialized();
}


float UUltraReticleWidgetBase::ComputeSpreadAngle() const
{
	if (const UUltraRangedWeaponInstance* RangedWeapon = Cast<const UUltraRangedWeaponInstance>(WeaponInstance))
	{
...
bool UUltraReticleWidgetBase::HasFirstShotAccuracy() const
{
	if (const UUltraRangedWeaponInstance* RangedWeapon = Cast<const UUltraRangedWeaponInstance>(WeaponInstance))
	{
...
float UUltraReticleWidgetBase::ComputeMaxScreenspaceSpreadRadius() const
{
