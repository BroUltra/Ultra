...
#include "UltraFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(UltraFrontendStateComponent)
...
UUltraFrontendStateComponent::UUltraFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UUltraFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UUltraFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool UUltraFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void UUltraFrontendStateComponent::OnExperienceLoaded(const UUltraExperienceDefinition* Experience)
{
...
void UUltraFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void UUltraFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &UUltraFrontendStateComponent::OnUserInitialized);
...
void UUltraFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &UUltraFrontendStateComponent::OnUserInitialized);
...
void UUltraFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void UUltraFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
