...
UENUM(BlueprintType)
enum class EUltraAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UUltraSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 *               maybe change Localsettings to UltraPlatformSettings, or DeviceSettings?  Make this one UserSettings?  TBD
 *               
...
class UUltraSettingsShared : public USaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UUltraSettingsShared, FOnSettingChangedEvent, UUltraSettingsShared* Settings);
	FOnSettingChangedEvent OnSettingChanged;

public:

	UUltraSettingsShared();

	void Initialize(UUltraLocalPlayer* LocalPlayer);

	bool IsDirty() const { return bIsDirty; }
	void ClearDirtyFlag() { bIsDirty = false; }

	void SaveSettings();
	static UUltraSettingsShared* LoadOrCreateSettings(const UUltraLocalPlayer* LocalPlayer);
...
public:
	UFUNCTION()
	EUltraAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EUltraAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EUltraAllowBackgroundAudioSetting AllowAudioInBackground = EUltraAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EUltraGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EUltraGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EUltraGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EUltraGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EUltraGamepadSensitivity GamepadLookSensitivityPreset = EUltraGamepadSensitivity::Normal;
	UPROPERTY()
	EUltraGamepadSensitivity GamepadTargetingSensitivityPreset = EUltraGamepadSensitivity::Normal;
...
private:
	...
	UPROPERTY(Transient)
	TObjectPtr<UUltraLocalPlayer> OwningPlayer = nullptr;
};
